...
import scala.quoted.{ Expr, QuoteContext, Reporting }
      ...
      case Apply(_, List(Apply(_, List(Typed(Repeated(List(lit @ Literal(Constant(str: String))), _), _))))) =>
        try {
          ...
          case xpee: XPathExpressionException =>
            Reporting.error(s"Illegal XPath expression: ${xpee.getMessage}")
            '{null}
        }
      case _ =>
        Reporting.error("xpath can only be used on string literals")
        '{null}
    }
  }
}
