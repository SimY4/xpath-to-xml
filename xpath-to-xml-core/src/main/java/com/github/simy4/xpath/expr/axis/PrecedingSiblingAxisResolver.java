...
public class PrecedingSiblingAxisResolver extends AbstractAxisResolver {
    ...
    protected <N extends Node> Iterable<? extends N> traverseAxis(Navigator<N> navigator, NodeView<N> view) {
        ...
        return null == parent ? Collections.<N>emptyList()
                : new PrecedingSiblingIterable<N>(navigator, parent, node, sibling);
    }
    ...
    public <N extends Node> NodeView<N> createAxisNode(Navigator<N> navigator, NodeView<N> node, int position)
        ...
        return new NodeView<N>(newElement, position);
    }
    ...
    private static final class PrecedingSiblingIterable<T extends Node> implements Iterable<T> {
        ...
        public Iterator<T> iterator() {
            final Iterator<T> preceding = new Preceding<T>(navigator.elementsOf(parent).iterator(), current);
            return sibling ? preceding : new DescendantOrSelfAxisResolver.DescendantOrSelf<T>(navigator, preceding);
        }

    }

    private static final class Preceding<T extends Node> extends ReadOnlyIterator<T> {
